...
void fun01();   //封装的多个线性表的处理函数
void fun02(LinkList &L );  //封装的处理单个线性表的处理函数
...
status ClearList(LinkList &L)
{
    ...
    if(L->next == NULL)
    {
        printf("线性表已经是空的了，不需要操作\n");
        return INFEASIBLE;
    }
    LinkList p = L->next;   //指向第一个元素节点
    while(p)
    {
        free(p);            //释放当前节点
        p = p->next;        //指向下一个节点
    }
    L->next = NULL;         //将头节点指向NULL，清空线性表
    return OK;
}
        ...
        switch (a) {
            ...
            case 4:
                ...
                if(judge ==-1) //如果查找失败
                {
                    ...
                    fun02(Lists.elem[judge-1].L); //执行另一个函数
                }
                break;
...
void fun02(LinkList &L)       //这个函数进行每个线性表的详细功能实现
{
    system("cls");  //清空命令行窗口
    printf("线性表存在鸭鸭\n");
    printf("现在对这个线性表进行操作\n");
